...
fertility <- read_csv("new fertility (Cut Down).csv")
lifeexpectancy <- read_csv("new life expectancy (Cut Down).csv")
education <- read_csv("new primary (Cut Down).csv")
...
LeftJoin2 <-  LeftJoin %>% 
  gather(key, value, c(-Date, -'Country Name'))

View(LeftJoin2)

#ggplot(LeftJoin2, mapping = aes(x = Date, y = value, group = key, colour = key)) + geom_line()
